# we can use patch size to down sample the data (we dont have to do it in the pre process step)
# Say we put in 512 ms of data, with a patch size of 4 using a PatchEmbedding1D layer, we would get 128 data points (token size of 4 ms) 512/4 = 128
# Using a CLS token (learnable embedding that is prepended to sequences of embdedded patches)
# use positional embeddings to retain the temporal information
# in dream diffusion they are using 24 encoder blocks, and 8 decoder blocks. They have input embedding of 1024 with out of 512 ( I dont know how that works)
# selecting an MLP ratio = size of hidden layer / embedding size
# They use an xaiver uniform initialization for the weights let's investigate this
...
class EEGTransformerAutoencoder(nn.Module):
    def __init__(self, num_channels, patch_size, num_heads=63, num_encoder_layers = 12, num_decoder_layers = 6, d_model = 128, dropout=0.1):
        super(EEGTransformerAutoencoder, self).__init__()

        # Encoder
        self.encoder_conv = nn.Conv1d(num_channels, d_model, kernel_size=patch_size, stride=patch_size)
        encoder_layers = nn.TransformerEncoderLayer(d_model, num_heads, dim_feedforward=d_model * 4, dropout=dropout, batch_first=True)
        self.transformer_encoder = nn.TransformerEncoder(encoder_layers, num_encoder_layers)

        # Decoder
        decoder_layers = nn.TransformerDecoderLayer(d_model, num_heads, dim_feedforward=d_model * 4, dropout=dropout, batch_first=True)
        self.transformer_decoder = nn.TransformerDecoder(decoder_layers, num_decoder_layers)
        self.decoder_conv = nn.ConvTranspose1d(d_model, num_channels, kernel_size=patch_size, stride=patch_size)

    def forward(self, x):
        # Encoder
        x = x.permute(0, 2, 1)  # (batch_size, time_series_length, num_channels)
        x = self.encoder_conv(x)
        x = x.permute(0, 2, 1)  # (batch_size, num_channels, d_model)
        encoded = self.transformer_encoder(x)

        # Decoder
        decoded = self.transformer_decoder(encoded, encoded)
        decoded = decoded.permute(0, 2, 1)  # (batch_size, d_model, num_channels)
        decoded = self.decoder_conv(decoded)
        decoded = decoded.permute(0, 2, 1)  # (batch_size, time_series_length, num_channels)

        return decoded
class PatchEmbed1D(nn.Module):
    """ 1 Dimensional version of data (fmri voxels) to Patch Embedding
    """
    def __init__(self, time_len=128, patch_size=1, in_chans=63, embed_dim=256):
        super().__init__()
...
class Encoder(nn.Module):
    def __init__(self, time_len=512, patch_size=4, embed_dim=1024, in_chans=128,
        ...
        self.patch_embed = PatchEmbed1D(time_len=time_len, patch_size=patch_size, in_chans=in_chans, embed_dim=embed_dim)
